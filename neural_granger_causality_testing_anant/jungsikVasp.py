...
def spatial_preprocess(directory):
    d = {'Edge Velocity':[], "VASP Mutant":[], "Actin":[], "VASP Wild": []}
    for filepath in os.listdir(directory):
        if('txt' in filepath):
            ...
            if('Chan0' in filepath):
                d['Edge Velocity'].append(df)
            elif('Chan1' in filepath):
                d['Actin'].append(df)
            elif('Chan2' in filepath):
                d['VASP Mutant'].append(df)
            elif('Chan3' in filepath):
                d['VASP Wild'].append(df)
    return [np.array(d["Edge Velocity"]), 
            np.array(d["Actin"]), np.array(d["VASP Mutant"]), np.array(d["VASP Wild"])]
...
def getNeighbors(arr, window, layer):
    ...
    for layer_delt, window_delt in [(-1,0), (0, -1), (0,0), (0,1), (1,0)]:
        ...
        if(layer_temp >=0 and layer_temp < 4):
            actins.append(arr[1][layer_temp][:, window_temp])
            mutants.append(arr[2][layer_temp][:, window_temp])
            wilds.append(arr[3][layer_temp][:, window_temp])
    actins = np.array(actins)
    mutants = np.array(mutants)
    wilds = np.array(wilds)
    final = np.concatenate((actins, mutants, wilds, edgeVelocities)).transpose()
...
def constructDataset(self, arr = None):
    ...
    assert numWindows == 42 and timeLen == 300 #hardcoded for Jungsiks data
    predictors = []
    responses = []
    for window in range(1, 43):
...
def plot(truth, layer, directory):
    ...
    mutants = ["Mut {}".format(i) for i in range(1,n)]
    wilds = ["Wild {}".format(i) for i in range(1,n)]
    EV = ["EV {}".format(i) for i in range(3)]
    ticks = actins+mutants+wilds+EV
...
def makeGCgraph2(self, x, singular_iteration, make_final_graph,plotSwitch = True, argument = 0.5):     
    ...
    with torch.no_grad():
        ...
        plot(self.graph_est, LAYER, "VASPImages")
...
if(__name__ == "__main__"):
    ...
    while(LAYER < 5):
        ...
        save_dir = "jungSikModels/{}".format(LAYER)
        arr = spatial_preprocess(data_dir)
        numVars = 15 if LAYER == 1 or LAYER == 4 else 18
        lstmtester=GVARTesterTRGC(arr, numvars = numVars, cuda=False)
        try:
            os.makedirs(save_dir+"/"+type(lstmtester).__name__)
        except:
            pass
        ...
        #lstmtester.save(save_dir+"/"+type(lstmtester).__name__)
        lstmtester.make_GC_graph2 = types.MethodType(makeGCgraph2, lstmtester)
        plot(lstmtester.make_GC_graph(arr), LAYER, save_dir+"/"+type(lstmtester).__name__)
